...
public class HtmlSanitizerHttpServletRequestWrapper extends HttpServletRequestWrapper {
    ...
    public HtmlSanitizerHttpServletRequestWrapper(HttpServletRequest request, String allowElements) throws IOException {
        ...
        if (originContent.length > 0) {
            ...
            if (contentType != null && !contentType.equals(MULTIPART_FORM_DATA_VALUE)) {
                cachedContent = htmlSanitizerProcessor.sanitize(new String(originContent, UTF_8)).getBytes(UTF_8);
            } else {
    ...
    private String sanitize(String originContent) {
        return htmlSanitizerProcessor.sanitize(originContent);
    }
}
