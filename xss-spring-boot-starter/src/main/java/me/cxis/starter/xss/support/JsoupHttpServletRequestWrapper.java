...
public class JsoupHttpServletRequestWrapper extends HttpServletRequestWrapper {
    ...
    public JsoupHttpServletRequestWrapper(HttpServletRequest request, String whitelist) throws IOException {
        ...
        if (originContent.length > 0) {
            ...
            if (contentType != null && !contentType.equals(MULTIPART_FORM_DATA_VALUE)) {
                cachedContent = jsoupProcessor.clean(new String(originContent, UTF_8)).getBytes(UTF_8);
            } else {
    ...
    private String cleanHtml(String originContent) {
        return jsoupProcessor.clean(originContent);
    }
}
